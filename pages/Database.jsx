...
export default function Database(){
 ...
 const renderItem=x=><article className="item" onClick={()=>window.dispatchEvent(new CustomEvent("aetheria:item",{detail:x}))}><div className="item-art">{x.image?<img src={x.image} alt="" loading="lazy"/>:<Sparkles/>}<span>#{x.id}</span></div><div className="item-body"><div className="item-title"><h3>{x.name}</h3><b>{x.rarity?'★'.repeat(Number(x.rarity)):''}</b></div><p>{x.type||'Chưa phân loại'} · {x.source||'Chưa xác định'}</p><div className="tags">{(x.tags||[]).map(t=><span key={t}>{t}</span>)}</div><div className="score"><span>Thuộc tính</span><strong>{Object.keys(x.attributes||{}).length||0}/10</strong></div></div></article>;
